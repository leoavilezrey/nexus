# Código Fuente de Nexus para Análisis Pre-Implementación

Este documento contiene el código actualizado de los archivos centrales que serán modificados en el Plan de Implementación.

## Archivo: `main.py`
**Ruta:** `c:\Users\DELL\Proyectos\nexus\main.py`
```python
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
def menu_gestionar(initial_query: str = ""):
    ...
    while True:
        ...
        console.print(table)
        console.print(
            "\n[bold yellow]Navegación Rápida:[/] [bold]← →[/] Páginas │ [bold]0[/] Menú Principal │ [bold][Enter][/] o [bold]:[/] Escribir Comando Complejo\n"
        )
        console.print(
            "[dim]Comandos Complejos: Q (Filtro) │ L (Limpiar) │ [ID] (Detalle) │ del [IDs] (Borrar) │ m ID1 ID2 (Vínculo IA)[/]\n"
        )

        # ── Híbrido: Captura con get_key pasivo
        cmd_lower = ""
        action = get_key()
        
        if action == '0':
            break
        elif action == '\x1b[C' or action == 'right' or action == '→' or action == 'm':  # Right Arrow
            if has_next: page += 1
            else:
                console.print("[yellow]Ya estás en la última página.[/]")
                time.sleep(0.8)
            continue
        elif action == '\x1b[D' or action == 'left' or action == '←' or action == 'k':  # Left Arrow
            if page > 0: page -= 1
            continue
        elif action in ('\r', '\n', ':'): 
            # Prompt de texto completo (fallback a Prompt.ask)
            cmd = Prompt.ask("[bold bright_cyan]Comando Libre ▶[/]", console=console).strip()
            cmd_lower = cmd.lower()
        elif action.isdigit():
            # Auto-escribir el dígito que presionaron y permitir expandirse
            cmd = Prompt.ask("[bold bright_cyan]ID/Comando ▶[/]", default=action, console=console).strip()
            cmd_lower = cmd.lower()
        elif action.isalpha() and ord(action) >= 32:
            # Capturaron una letra
            cmd = Prompt.ask("[bold bright_cyan]Comando Libre ▶[/]", default=action, console=console).strip()
            cmd_lower = cmd.lower()
        else:
            continue
        ...
        # Filtros
        elif cmd_lower == 'q' or cmd_lower == 'q ':
        ...
        if opcion == "0":
            ...
            while True:
                ...
                if cmd == '0':
                    ...
                    tid = int(cmd)
                    from modules.study_engine import start_pomodoro_session
...
def menu_active_recall():
    ...
    while True:
        ...
        console.print(
            "[dim]Comandos Complejos: Q (Filtro) │ L (Limpiar) │ [ID] (Entrar) │ ia [IDs] (Flashcards) │ pm (Pomodoro) │ pa (Adelantar) │ del [IDs][/]\n"
        )
        ...
        if action == '0':
            ...
            cmd = Prompt.ask("[bold bright_cyan]Recall Libre ▶[/]", console=console).strip()
            cmd_lower = cmd.lower()
        elif action.isdigit():
            cmd = Prompt.ask("[bold bright_cyan]ID/Comando ▶[/]", default=action, console=console).strip()
            cmd_lower = cmd.lower()
        elif action.isalpha() and ord(action) >= 32:
            cmd = Prompt.ask("[bold bright_cyan]Recall Libre ▶[/]", default=action, console=console).strip()
            cmd_lower = cmd.lower()
        else:
            continue
        ...
        if cmd_lower == '0':
            ...
            console.print("\n[bold yellow]🍅 Configuración de Sesión Pomodoro[/]")
            topic_str = Prompt.ask("¿Deseas estudiar un Tema Específico? (Ingresa el [bold]ID[/] o presiona [bold]Enter[/] para Mezcla Global)").strip()
            
            target_topic_id = None
            if topic_str.isdigit():
                target_topic_id = int(topic_str)
                # Validar existencia
                with nx_db.Session() as tmp_s:
                    if not tmp_s.query(Registry).filter(Registry.id == target_topic_id).first():
                        console.print(f"[bold red]❌ El Tema con ID {target_topic_id} no existe.[/]")
                        time.sleep(1.5)
                        continue
                        
            from modules.study_engine import start_pomodoro_session
            start_pomodoro_session(pomodoro_minutes=25, adelantar=False, topic_id=target_topic_id)
            ...
            if raw_id.isdigit():
                ...
                if reg_obj:
                    ...
                    from modules.study_engine import open_source_material
